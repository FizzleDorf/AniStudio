...
namespace GUI {
ExampleView::ExampleView(ECS::EntityManager &entityMgr) : BaseView(entityMgr) { viewName = "Example Plugin View"; }

ExampleView::~ExampleView() = default;

void ExampleView::Render() {
    ImGui::Begin(viewName.c_str());
    if (ImGui::Button("Create Counter")) {
        auto entity = mgr.AddNewEntity();
        mgr.AddComponent<ECS::ExampleComponent>(entity);
    }
    ImGui::Separator();
