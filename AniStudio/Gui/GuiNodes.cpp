...
void Node::AddFloatInput(const std::string &key, float value) { floatInputs[key] = value; }

void Node::AddStringInput(const std::string &key, const std::string &value) { stringInputs[key] = value; }

void Node::AddArrayInput(const std::string &key, const std::vector<std::string> &arrayValue) {
    arrayInputs[key] = arrayValue;
}

const std::string &Node::GetClassType() const { return classType; }

void Node::SetClassType(const std::string &type) { classType = type; }

const std::string &Node::GetTitle() const { return title; }

void Node::SetTitle(const std::string &title) { this->title = title; }

const std::unordered_map<std::string, float> &Node::GetFloatInputs() const { return floatInputs; }

const std::unordered_map<std::string, std::string> &Node::GetStringInputs() const { return stringInputs; }

const std::unordered_map<std::string, std::vector<std::string>> &Node::GetArrayInputs() const { return arrayInputs; }

void Node::UpdateValue(const std::string &key, float value) { floatInputs[key] = value; }

void Node::UpdateValue(const std::string &key, const std::string &value) { stringInputs[key] = value; }
