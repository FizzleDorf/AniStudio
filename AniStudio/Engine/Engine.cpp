...
namespace ANI {
    ...
    static VkPhysicalDevice SetupVulkan_SelectPhysicalDevice()
    {
        ...
        // Default to CPU (integrated GPU) instead of discrete GPU
        ...
        // If no integrated GPU is found, fall back to any available device
        if (!gpus.empty())
            return gpus[0];
...
void ANI::Engine::Update() {
    ...
    // Add ImGui widgets and UI elements here
    {
        ...
        if (viewState.showDiffusionView) diffusionView.Render();
        // if (viewState.showDrawingCanvas) canvasView.Render();
        if (viewState.showSettingsView) settingsView.Render();
