...
namespace Utils
{
	...
	public:
		...
		static std::string CreateUniqueFilename(const std::string &baseFilename, const std::string &directory,
			const std::shared_ptr<FilePaths>& filePaths = nullptr)
		{
			try
			{
				// Validate inputs
				...
				if (validDirectory.empty() && filePaths) {
					validDirectory = filePaths->GetPath("DefaultProject");
				}
				else if (validDirectory.empty()) {
					// Fallback if no FilePaths instance provided
					validDirectory = ".";
				}

				// Ensure directory exists
				std::filesystem::path directoryPath(validDirectory);
				if (!directoryPath.is_absolute() && std::filesystem::path(validDirectory).is_relative())
				{
