/*
 * Fixed VideoView.hpp - No Direct ECS System Callbacks
 * CRITICAL FIX: VideoView should NOT register callbacks with ECS systems
...
namespace GUI {
	...
	public:
		...
		void Init() override {
			...
			// CRITICAL FIX: Only ensure VideoSystem exists - NO CALLBACK REGISTRATION
			...
			std::cout << "[VideoView] Initialization complete - NO CALLBACKS REGISTERED" << std::endl;
		}
		...
		void Update(float deltaT) override {
			...
			if (videoSystem) {
				...
				if (currentEntities.size() != lastEntityCount) {
					std::cout << "[VideoView] Entity count changed: " << lastEntityCount
						<< " -> " << currentEntities.size() << std::endl;

					videoEntities = currentEntities;
					...
					if (videoEntities.empty()) {
						...
						// Update videoIndex if selected entity still exists
	...
	private:
		...
		void RefreshVideoEntities() {
			try {
				auto videoSystem = mgr.GetSystem<ECS::VideoSystem>();
				if (videoSystem) {
					videoEntities = videoSystem->GetAllVideoEntities();
					lastEntityCount = videoEntities.size();
					...
					videoEntities.clear();
					lastEntityCount = 0;
					std::cout << "[VideoView] No VideoSystem found, cleared entity list" << std::endl;
				}
			}
			catch (const std::exception& e) {
