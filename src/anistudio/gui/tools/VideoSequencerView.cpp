...
namespace GUI {
	...
	int VideoSequence::GetFrameMin() const {
		return frameMin;
	}

	int VideoSequence::GetFrameMax() const {
		return frameMax;
	}

	int VideoSequence::GetItemCount() const {
		return static_cast<int>(clips.size());
	}

	const char* VideoSequence::GetItemTypeName(int typeIndex) const {
		switch (typeIndex) {
	...
	const char* VideoSequence::GetItemLabel(int index) const {
		return clips[index].name.c_str();
	}

	void VideoSequence::Get(int index, int** start, int** end, int* type, unsigned int* color) {
	...
	void VideoSequence::Add(int type) {
		VideoClip newClip;
		newClip.type = type;
	...
	void VideoSequence::Del(int index) {
		if (index >= 0 && index < clips.size()) {
	...
	void VideoSequence::Duplicate(int index) {
		if (index >= 0 && index < clips.size()) {
	...
	void VideoSequence::Copy() {
		// Not implemented yet
	}

	void VideoSequence::Paste() {
		// Not implemented yet
	}

	size_t VideoSequence::GetCustomHeight(int index) {
		return 24;  // Slightly taller height for thumbnails
	}

	void VideoSequence::CustomDraw(int index, ImDrawList* draw_list, const ImRect& rc, const ImRect& legendRect,
	...
	void VideoSequence::CustomDrawCompact(int index, ImDrawList* draw_list, const ImRect& rc, const ImRect& clippingRect) {
		if (index >= 0 && index < clips.size()) {
	...
	void VideoSequence::BeginEdit(int index) {
		selectedEntry = index;
	}

	void VideoSequence::EndEdit() {
		// Save changes if needed
	}

	int VideoSequence::GetItemTypeCount() const {
		return TYPE_COUNT;  // Number of clip types
	}

	void VideoSequence::DoubleClick(int index) {
		// Handle double click - select and focus on the clip
		selectedEntry = index;
	}

	const char* VideoSequence::GetCollapseFmt() const {
		return "%d Frames / %d Elements";
	}

	// VideoSequencerView implementation
	VideoSequencerView::VideoSequencerView(ECS::EntityManager& entityMgr)
		: BaseView(entityMgr),
		currentFrame(0),
		...
		videoFilters("Video files{.mp4,.avi,.mkv,.mov,.webm};"),
		imageFilters("Image files{.jpg,.jpeg,.png,.bmp,.tga};")
	{
	...
	VideoSequencerView::~VideoSequencerView() {
	...
	void VideoSequencerView::Render() {
	...
	void VideoSequencerView::RenderPreview() {
		ImVec2 contentSize = ImGui::GetContentRegionAvail();
	...
	void VideoSequencerView::RenderTimeline() {
	...
	void VideoSequencerView::RenderClipProperties() {
		if (sequence.selectedEntry < 0 || sequence.selectedEntry >= sequence.clips.size()) {
			return;
		}
	...
	void VideoSequencerView::RenderMediaBrowser() {
	...
	void VideoSequencerView::RenderVideoBrowser() {
	...
	void VideoSequencerView::RenderImageBrowser() {
		if (ImGui::Button("Refresh")) {
			UpdateAvailableMedia();
		}
	...
	ImVec2 VideoSequencerView::CalculatePreviewSize(const ImVec2& availableSize) {
	...
	std::string VideoSequencerView::FormatTime(float seconds) {
	...
	void VideoSequencerView::InitializePreviewTexture() {
	...
	void VideoSequencerView::UpdatePreviewAtFrame(int frame) {
	...
	void VideoSequencerView::UpdatePreviewFromVideoClip(const VideoSequence::VideoClip& clip, int clipFrame) {
	...
	void VideoSequencerView::UpdatePreviewFromImageClip(const VideoSequence::VideoClip& clip) {
	...
	void VideoSequencerView::CopyTexture(GLuint sourceTexture, GLuint destTexture) {
	...
	void VideoSequencerView::ClearPreview() {
		if (previewTexture != 0) {
			glBindTexture(GL_TEXTURE_2D, previewTexture);
	...
	void VideoSequencerView::HandleUpdate() {
	...
	void VideoSequencerView::AddImageToSequence(ECS::EntityID entityID) {
		if (!mgr.HasComponent<ECS::ImageComponent>(entityID)) {
			return;
		}
	...
	void VideoSequencerView::ReplaceClipMedia(int clipIndex) {
	...
	void VideoSequencerView::HandleVideoAdded(ECS::EntityID entityID) {
		// Update available videos list
		UpdateAvailableMedia();
	}

	void VideoSequencerView::HandleVideoRemoved(ECS::EntityID entityID) {
		// Update available videos list
		UpdateAvailableMedia();
