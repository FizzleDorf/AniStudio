...
namespace GUI {
    ...
    void ImageView::RenderHistory() {
        ...
        for (size_t i = 0; i < imageEntities.size(); ++i) {
            ...
            try {
                ...
                RenderImageContextMenu(entityID);
    ...
    void ImageView::RenderImageContextMenu(ECS::EntityID entityID) {
        if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(ImGuiMouseButton_Right)) {
            ImGui::OpenPopup(("ImageContextMenu_" + std::to_string(entityID)).c_str());
        }

        if (ImGui::BeginPopup(("ImageContextMenu_" + std::to_string(entityID)).c_str())) {
